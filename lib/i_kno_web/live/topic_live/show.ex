defmodule IKnoWeb.TopicLive.Show do
  ...
  def mount(%{"subject_id" => subject_id}, %{"user_token" => user_token}, socket) do
    ...
    socket = assign(socket, subject: subject, user: user, is_admin: is_admin, take_question: false)
  ...
  def handle_event("post-question", _, socket) do
    socket = assign(socket, take_question: false)
    {:noreply, socket}
  end
  ...
  def render(assigns) do
    ...
    <% else %>
      <div>
        ...
        <form :if={@take_question} phx-submit="post-question" class="mt-5">
