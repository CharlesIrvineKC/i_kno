defmodule IKnoWeb.TopicLive.FormComponent do
  ...
  def update(%{topic: topic} = assigns, socket) do
    changeset = Knowledge.change_topic(topic)
  ...
  def handle_event("validate", %{"topic" => topic_params}, socket) do
    changeset =
      ...
      |> Knowledge.change_topic(topic_params)
  ...
  defp save_topic(socket, :edit, topic_params) do
    case Knowledge.update_topic(socket.assigns.topic, topic_params) do
      {:ok, topic} ->
  ...
  defp save_topic(socket, :new, topic_params) do
    case Knowledge.create_topic(topic_params) do
      {:ok, topic} ->
        notify_parent({:saved, topic})
